xml.instruct! :xml, :version => "1.0"
  xml.rss "version" => "2.0", "xmlns:dc" => "http://purl.org/dc/elements/1.1/" do
    xml.channel do
      domain = "https://#{locals[:city].domain}.#{request.main_domain}"
      ...
      locals[:screenings].each do |screening|
        ...
        description = "#{show_date_time_feed(screening.date_time)}.<br/>«<a target=\"_blank\" href=\"" + domain + url_for(:movies, :index, :id => format_movie_url(movie)) + "\">#{movie.title}</a>» в кинотеатре #{cinema.name}.<br/><img src=\"#{poster_url}\">"
        xml.item do
          xml.title title
          xml.description do
