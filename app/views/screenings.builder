...
xml.rss 'version' => '2.0', 'xmlns:dc' => 'http://purl.org/dc/elements/1.1/' do
  xml.channel do
    ...
    locals[:screenings].each do |screening|
      ...
      next if movie.nil? || cinema.nil?

      title = movie.title
      title += " (#{format_title(movie.title_original)})" unless movie.title_original.to_s.empty?
      description = "#{show_date_time_feed(screening.date_time)}.<br/>«<a target=\"_blank\" href=\"" + \
                    domain + url_for(:movies, :index, id: format_movie_url(movie)) + \
                    "\">#{movie.title}</a>» в кинотеатре #{cinema.name}.<br/><img src=\"#{movie.poster_url}\">"
      xml.item do
        xml.title title
        xml.description do
